...
本次只记录优化点，不直接改动功能代码。优先级主要按真实收益、维护风险和实施成本综合排序。
...
| 一言请求空闲调度 | `src/scripts/hitokoto.ts` | 首页一言在没有同日缓存时改为 `requestIdleCallback` 或短 timeout 后再请求，避免 cache miss 立即进入首屏关键路径。 |
...
| P1 | 外链图片 URL 严格校验 | `src/utils/photoWall.ts`, `tests/photo-wall.test.ts` | 域名 allowlist 已落地，但 `http://` 当前仍会通过允许域名检查，格式异常的 `https` URL 也可能抛出原生 `URL` 错误。建议进一步强制外链使用 `https`，并把 malformed URL 转成稳定、可测试的照片墙校验错误。 |
...
| P2 | 动态 OG 缓存失败恢复 | `src/utils/ogFont.ts` | 字体缓存已落地，但当前 cache 存的是 fetch promise。如果某次字体请求失败，失败 promise 会留在 cache 中。建议在 `.catch()` 时删除对应 cache key，让后续请求可以重试。 |
| P2 | 内容查询复用 | `src/pages/index.astro`, `src/pages/posts/[...page].astro`, `src/pages/tags/**`, `src/pages/rss.xml.ts`, `src/pages/archives/index.astro` | 多个路由重复 `getCollection`、过滤和排序。可以新增 `src/utils/contentQueries.ts`，用模块级 promise 复用已发布文章/动态查询，保持现有 `postFilter`、`getSortedPosts` 和 `getSortedMoments` 规则。 |
...
| P3 | Hitokoto 超时与开关 | `src/components/home/HitokotoCard.astro`, `src/scripts/hitokoto.ts`, `src/types/config.ts` | 一言 cache miss 已改为空闲调度。后续可给 `requestIdleCallback` 增加 timeout，并加入配置开关，方便隐私优先或离线部署直接关闭第三方请求。 |
...
1. **先处理安全与内容可靠性**：CMS 权限、外链图片 `https` 强制与 malformed URL 测试、slug 唯一性检查。
2. **再处理高收益资源优化**：照片墙响应式图片管线、CMS runtime 版本维护、Markdown 图片 transformer 测试。
3. **然后处理构建与脚本维护性**：OG 字体缓存失败恢复、内容查询复用、搜索脚本抽离、首页聚合逻辑抽离。
4. **最后处理长期体验优化**：RSS 阅读器实测、Giscus 占位高度、Hitokoto timeout/开关、字体预加载审计、体积观察脚本。
